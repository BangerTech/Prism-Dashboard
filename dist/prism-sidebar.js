class PrismSidebarCard extends HTMLElement {
    ...
    updateCamera() {
        ...
        if (streamContainer && cameraState && this.cameraLiveStream) {
            if (!this._cameraStreamElement || this._currentStreamEntity !== cameraEntity) {
                // Remove old stream element if entity changed
                if (this._cameraStreamElement) {
                    this._cameraStreamElement.remove();
                }
                // Create ha-camera-stream via DOM API (not innerHTML)
                ...
                console.log('[Prism Sidebar] Created ha-camera-stream for', cameraEntity);
            } else {
                // Update existing stream element with latest hass/state
    ...
    async updateForecastGrid() {
        ...
        if (!forecastGridEl || !weatherState) {
            console.log('[Prism Sidebar] Forecast: No grid element or weather state', { 
        ...
        const supportedFeatures = weatherState.attributes.supported_features || 0;
        console.log('[Prism Sidebar] Weather entity:', this.weatherEntity, {
        ...
        if (this.isLegacyWeather()) {
            console.log('[Prism Sidebar] Using legacy forecast from attributes');
            ...
            console.log('[Prism Sidebar] Using modern forecast subscription, type:', forecastType);
            
            try {
                // Subscribe to forecast updates
                const callback = (event) => {
                    console.log('[Prism Sidebar] Forecast callback received:', event);
                    if (event && event.forecast && Array.isArray(event.forecast)) {
                        let forecastData = event.forecast;
                        console.log('[Prism Sidebar] Forecast data received:', forecastData.length, 'items');
                ...
                console.log('[Prism Sidebar] Subscribing to forecast:', message);
                this.forecastSubscriber = await this._hass.connection.subscribeMessage(callback, message, { resubscribe: false });
                console.log('[Prism Sidebar] Subscription successful');
                
                // Wait a bit for the first callback
                await new Promise(resolve => setTimeout(resolve, 500));
                
            } catch (error) {
                console.warn('[Prism Sidebar] Forecast subscription failed:', error);
                // Fallback to attributes if subscription fails
                if (weatherState.attributes.forecast && weatherState.attributes.forecast.length > 0) {
                    forecast = weatherState.attributes.forecast;
                    console.log('[Prism Sidebar] Falling back to legacy forecast, items:', forecast.length);
                }
            }
        }
        ...
        if (forecast && forecast.length > 0) {
            console.log('[Prism Sidebar] Rendering forecast with', forecast.length, 'items');
            this.renderForecastGrid(forecast, forecastGridEl);
        } else {
            console.log('[Prism Sidebar] No forecast data yet, waiting for subscription callback...');
            ...
            setTimeout(() => {
                ...
                if (currentContent.includes('mdi:loading')) {
                    console.log('[Prism Sidebar] Forecast timeout - still no data');
        ...
        <div class="sidebar">
            ...
            <div class="weather-box">
                ...
                <div class="forecast-grid">
                    <div style="grid-column: 1 / -1; text-align: center; color: rgba(255,255,255,0.4); padding: 12px 0; font-size: 12px;">
